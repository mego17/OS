...
class Scheduler:
    ...
    def get_task_to_execute(self):
        ...
        priorities = [task.priority for task in self.execute_list]
        highest_priority_index = priorities.index(min(priorities))
        return self.execute_list[highest_priority_index]
    ...
    def execute(self, task):
        ...
        self.check_deadline(end_time + 1)
        self.current_time = end_time
        self.update_tasks_remaining_execution()

    def get_time_to_stop(self, task):
        next_task = self.get_task_to_execute()  
        if next_task and next_task.release_time <= self.current_time + task.remaining_execution:
            return self.current_time + task.remaining_execution
        else:
            return min(task.deadline, self.max_time)
